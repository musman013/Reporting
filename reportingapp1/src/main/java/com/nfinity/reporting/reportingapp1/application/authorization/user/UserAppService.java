...
public class UserAppService implements IUserAppService {
	...
	public void checkProperties(List<String> list) throws Exception  {
		for (int i = 0; i < list.size(); i++) {
			if(!(
				...
				list.get(i).replace("%20","").trim().equals("passwordTokenExpiration") ||
	...
	public BooleanBuilder searchKeyValuePair(QUserEntity user, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			...
			if(details.getKey().replace("%20","").trim().equals("passwordTokenExpiration")) {
				if(details.getValue().getOperator().equals("equals") && SearchUtils.stringToDate(details.getValue().getSearchValue()) !=null)
					builder.and(user.passwordTokenExpiration.eq(SearchUtils.stringToDate(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("notEqual") && SearchUtils.stringToDate(details.getValue().getSearchValue()) !=null)
					builder.and(user.passwordTokenExpiration.ne(SearchUtils.stringToDate(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   Date startDate= SearchUtils.stringToDate(details.getValue().getStartingValue());
				   Date endDate= SearchUtils.stringToDate(details.getValue().getEndingValue());
				   if(startDate!=null && endDate!=null)	 
					   builder.and(user.passwordTokenExpiration.between(startDate,endDate));
				   else if(endDate!=null)
					   builder.and(user.passwordTokenExpiration.loe(endDate));
                   else if(startDate!=null)
                	   builder.and(user.passwordTokenExpiration.goe(startDate));  
                 }
	...
	public Map<String,String> parseDashboardJoinColumn(String keysString) {
		
		Map<String,String> joinColumnMap = new HashMap<String,String>();
		joinColumnMap.put("userId", keysString);
		return joinColumnMap;
	}
	public Map<String,String> parseReportJoinColumn(String keysString) {
